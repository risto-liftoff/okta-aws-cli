...
func (w *WebSSOAuthentication) authorizeWithBrowser() (*okta.AccessToken, error) {
	...
	handler := http.HandlerFunc(func(rw http.ResponseWriter, r *http.Request) {
		...
		if errParam := q.Get("error"); errParam != "" {
			...
			fmt.Fprintf(rw, "<html><body><h1>Authorization Failed</h1><p>%s: %s</p><p>You may close this window.</p></body></html>",
		...
		if code == "" {
			...
			fmt.Fprint(rw, "<html><body><h1>Error</h1><p>No authorization code received.</p></body></html>")
			return
		}

		resultCh <- callbackResult{code: code, state: returnedState}
		fmt.Fprint(rw, "<html><body><h1>Authorization Successful</h1><p>You may close this window and return to the CLI.</p></body></html>")
	})
